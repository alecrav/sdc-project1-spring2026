...
unsigned dynamatic::getOpDatawidth(Operation *op) {
  ...
  return llvm::TypeSwitch<Operation *, unsigned>(op)
      ...
      .Case<memref::StoreOp>([&](auto) {
        Type oprd0Type = op->getOperand(0).getType();
        unsigned oprd0width =
            isa<IndexType>(oprd0Type) ? 32 : oprd0Type.getIntOrFloatBitWidth();

        return oprd0width;
      })
      .Case<Operation *>([&](auto) {
        // Default case: assume the datawidth is that of the first result
        assert(op->getNumResults() > 0 &&
               "Operation has no results to extract datawidth from");
        return getHandshakeTypeBitWidth(op->getResult(0).getType());
      })
      .Default([&](auto) {
